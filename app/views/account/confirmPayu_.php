...
if ($_POST['state_pol'] == 4) {
    ...
    $update = $_DB->prepare("UPDATE registro SET wplay_l3.registro.creditos = wplay_l3.registro.creditos + :cre WHERE wplay_l3.registro.usuario_id = ?");
    $idUser->execute(array(':cre' => $valor, ':u' => $usuario_id));
}
if ($_POST['state_pol'] == 4) {

    $deposito = explode(" ", $_POST['referenceCode']);
    ...
    $update = $_DB->prepare("UPDATE registro SET wplay_l3.registro.creditos = wplay_l3.registro.creditos + ? WHERE wplay_l3.registro.usuario_id = ?");
    ...
    if ($idUser->status == 'draft_enviado') {
        ...
        $configTuCompra->execute(array(':origen' => 4));
        ...
        // $stmt = $_DB->prepare("UPDATE registro SET creditos_base=creditos_base+ :val WHERE usuario_id= :u");
        // $stmt->execute(array(':val' => $_POST['value']));


        // $valor = puntos($_POST['valor']);
        $mediopago_id = 1;

        //Valida los numeros
        /*$seguir = true;

        if (!ValidarCampo($usuario_id, "S", "N", 20)) {
            $seguir = false;
        }

        if (!ValidarCampo($valor, "S", "N", 15)) {
            $seguir = false;
        } else {
            if (floatval($valor) <= 0) {
                $seguir = false;
        ...
        /*if ($validaSql->rowCount() == 0) {
            $result = array('state' => 0, 'msg' => 'No fue posible ubicar el cliente para la recarga.');
            echo json_encode($result);
        } else if ($validaSql->rowCount() == 1) {*/
        ...
        if (!$updateEstado->execute(array(':id' => $deposito[1], ':s' => 4))) {
            $error .= 'Error factura';
        }
...
// ob_start();
// require_once('template_mail/recarga/mailer.php');
